from numpy import *
from ENIDriftensemble import *
from GeneratingIndex import *

class ENIDrifttrain():
    
    def __init__(self, lamda=None, delta=None, size=[700, 5, 700, 5],
                 num_test=10, learner='PCA', mute=True, incremental=True):
        
        self.G_idx_normal = R_index(lamda, delta[0], num_test = size[0], ID='normal')
        ...
        self.detector = dual_ensemble(learner)
    ...
    #            if not self.mute:
    #                print("Bad generateing Set: when update normal class...")
                self.G_idx_normal.clear()
                self.normal_set = array([])
                print("Overflow normal")
            else:
                if self.G_idx_normal.check(self.normal_set) == 'generateing':
                    data_temp = self.G_idx_normal.get()
                    print('normal generate'+str(data_temp.shape))
