...
class ENIDrift_train():
    ...
    def __init__(self, lamda=None, delta=None, size=[700, 5, 700, 5],
        ...
        self.retrain_size_normal = [size[0], size[1]]
        self.num_test = num_test
        self.learner = learner
        self.mute = True
        self.incremental = incremental
        self.retrain_temp = array([])
        self.num_retrain_temp = 0
    ...
    def predict(self, x):
        ...
        if self.num_retrain_temp != 0:
            self.retrain_temp = append(self.retrain_temp, x, axis=0)
        else:
            self.retrain_temp = x
            self.num_retrain_temp = 1
    ...
    def update(self, y):
        ...
        normal_temp = array([self.retrain_temp[i_y] for i_y in range(y.shape[0]) if y[i_y] == 0])
        attack_temp = array([self.retrain_temp[i_y] for i_y in range(y.shape[0]) if y[i_y] != 0])
        ...
        if normal_temp.shape[0]!= 0:
            ...
            # Update normal class
            if self.normal_set.shape[0] < self.retrain_size_normal[0]:
                pass
            elif self.retrain_size_normal[1] <= self.G_idx_normal.times():
                self.G_idx_normal.clear()
                self.normal_set = array([])
            else:
                if self.G_idx_normal.check(self.normal_set) == 'Training':
                    data_temp = self.G_idx_normal.get()
                    self.detector.retrain('normal', data_temp)
                self.normal_set = array([])
        ...
        if attack_temp.shape[0] != 0:
            ...
            # Update attack class
            if self.attack_set.shape[0] < self.retrain_size_normal[0]:
                pass
            else:
